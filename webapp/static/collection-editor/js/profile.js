...
function validateProfileData(data) {
  ...
  if (data.unlocked || data.discovered || data.alerted) {
    return {
      valid: false,
      error:
        "This is a meta.jkr file, not a profile.jkr file. Please select your profile.jkr file instead.",
    };
  }
  ...
  if (data.progress) {
    if (!data.progress.challenges)
      errors.push("progress.challenges is missing");
    if (!data.progress.deck_stakes)
      errors.push("progress.deck_stakes is missing");
  }
...
function toggleEditMode() {
  ...
  const allInputs = document.querySelectorAll(
    ".profile-input, .stat-value-input, .progress-input, .editable-stat",
  );
        ...
        <div class="profile-container">
            <div class="profile-header">
                <div class="profile-name-section">
                    <h2><i style="color:var(--accent-primary)" class="fa-solid fa-crown"></i> Player Profile</h2>
...
function renderCareerStats() {
  ...
  const statItems = [
    ...
    },
    {
      key: "c_jokers_sold",
      label: "Jokers Sold",
      icon: '<i class="fa-solid fa-theater-masks"></i>',
    },
    {
      key: "c_vouchers_bought",
...
function renderDeckUsage() {
  ...
  return Object.keys(decks)
    .map((deckId) => {
      ...
      const totalWins = Object.values(deck.wins || {}).reduce(
        (a, b) => a + b,
        0,
      );
      const totalLosses = Object.values(deck.losses || {}).reduce(
        (a, b) => a + b,
        0,
      );
      const winRate =
  ...
  return `
        <table class="stats-table">
            ...
            <tbody>
                ${jokers
                  .map(([id, data], index) => {
                    const wins = Object.values(data.wins || {}).reduce(
                      (a, b) => a + b,
                      0,
                    );
                    const losses = Object.values(data.losses || {}).reduce(
                      (a, b) => a + b,
                      0,
                    );
                    const winRate =
...
function renderHandUsage() {
  ...
  const hands = Object.entries(profileData.hand_usage).sort(
...
function attachProfileEventListeners() {
  ...
  document
    .querySelectorAll(".stat-value-input, .progress-input, .editable-stat")
    .forEach((input) => {
