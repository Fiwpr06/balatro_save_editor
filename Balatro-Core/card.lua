...
function Card:use_consumeable(area, copier)
    ...
    if self.ability.name == 'The Wheel of Fortune' or self.ability.name == 'Ectoplasm' or self.ability.name == 'Hex' then
        local temp_pool =   (self.ability.name == 'The Wheel of Fortune' and self.ability.eligible_strength_jokers) or 
                            ((self.ability.name == 'Ectoplasm' or self.ability.name == 'Hex') and self.ability.eligible_editionless_jokers) or {}
...
function Card:can_use_consumeable(any_state, skip_check)
    ...
    if G.STATE ~= G.STATES.HAND_PLAYED and G.STATE ~= G.STATES.DRAW_TO_HAND and G.STATE ~= G.STATES.PLAY_TAROT or any_state then
        ...
        if self.ability.name == 'The Wheel of Fortune' then 
            if next(self.ability.eligible_strength_jokers) then return true end
        end
        if self.ability.name == 'Ankh' then
        ...
        if self.ability.name == 'Ectoplasm' or self.ability.name == 'Hex' then 
            if next(self.ability.eligible_editionless_jokers) then return true end
        end
        if self.ability.name == 'The Emperor' or self.ability.name == 'The High Priestess'  then 
        ...
        G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.4, func = function()
            ...
            G.E_MANAGER:add_event(Event({trigger = 'after', delay = 1.3*math.sqrt(G.SETTINGS.GAMESPEED), blockable = false, blocking = false, func = function()
                ...
                for i = 1, _size do
                    ...
                    elseif self.ability.name:find('Standard') then
                        card = create_card((pseudorandom(pseudoseed('stdset')) > 0.6) and "Enhanced" or "Base", G.pack_cards, nil, nil, nil, true, nil, 'sta')
...
function Card:update(dt)
    ...
    if G.STAGE == G.STAGES.RUN then
        ...
        if self.ability.name == 'The Wheel of Fortune' then
            self.ability.eligible_strength_jokers = EMPTY(self.ability.eligible_strength_jokers)
            for k, v in pairs(G.jokers.cards) do
                if v.ability.set == 'Joker' and (not v.edition) then
                    table.insert(self.ability.eligible_strength_jokers, v)
                end
            end
        end
        if self.ability.name == 'Ectoplasm' or self.ability.name == 'Hex' then
            self.ability.eligible_editionless_jokers = EMPTY(self.ability.eligible_editionless_jokers)
            for k, v in pairs(G.jokers.cards) do
                if v.ability.set == 'Joker' and (not v.edition) then
                    table.insert(self.ability.eligible_editionless_jokers, v)
                end
            end
        end
