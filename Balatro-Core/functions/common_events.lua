        ...
        G.E_MANAGER:add_event(Event({
            ...
            func = function()
                ...
                elseif string.find(config.name, 'joker') then
                    table.insert(left_text, {n=G.UIT.O, config={object = DynaText({string = {config.card.ability.name}, colours = {G.C.FILTER}, shadow = true, pop_in = 0, scale = 0.6*scale, silent = true})}})
...
function create_card(_type, area, legendary, _rarity, skip_materialize, soulable, forced_key, key_append)
    ...
    if not forced_key and soulable and (not G.GAME.banned_keys['c_soul']) then
        ...
        not (G.GAME.used_jokers['c_soul'] and not next(find_joker("Showman")))  then
            if pseudorandom('soul_'.._type) > 0.997 then
                forced_key = 'c_soul'
            end
        end
